...
public:
	...
	void ChangePosition(XMFLOAT3 pos);
	...
	// ビルボードを回転させて描画
	void DrawRotateBillBoard(const DirectX::XMFLOAT4X4 & cameramat , float radian , int axis);
	...
	// 虹色
	void ChangeColor(float wait_time);

	// SRVセット
	void SetSRV(ID3D11ShaderResourceView * SRV);

	// 行列を入れる用
	DirectX::XMFLOAT4X4 m_billboard_mtx;
	void SetBillboardMtx()
	{
		m_mtx = m_billboard_mtx;
	}
