...
void CBillBoard::SetPosition(XMFLOAT3 pos)
{
    m_x = pos.x;
    m_y = pos.y;
    m_z = pos.z;
...
XMFLOAT3 CBillBoard::GetPosition()
{
    XMFLOAT3 billboard_pos;
...
void CBillBoard::ChangePosition(XMFLOAT3 pos)
{
    m_x += pos.x;
    m_y += pos.y;
    m_z += pos.z;
...
// ビルボードを描画( 1 でX軸、2 でY軸、3 でＺ軸 回転)
void CBillBoard::DrawRotateBillBoard(const DirectX::XMFLOAT4X4 & cameramat , float angle , int axis)
{
    ...
    DirectX::XMFLOAT3 axisZ = { cameramat._12, cameramat._22, cameramat._32 };

    switch (axis)
    {
        case 1:
            axisZ = { cameramat._11, cameramat._21, cameramat._31 };
            break;
        case 2:
            axisZ = { cameramat._12, cameramat._22, cameramat._32 };
            break;
        case 3:
            axisZ = { cameramat._13, cameramat._23, cameramat._33 };
            break;
        default:
            break;
    }
...
void CBillBoard::ChangeColor(float wait_time)
{
    if (change_color_num == 0)
    {
        m_Color = { 1.0f,0.0f,0.0f,1.0f };
    }
    else if (change_color_num <= 10 * wait_time)
    {
        m_Color.x -= 0.1f / wait_time;
        m_Color.y += 0.1f / wait_time;
    }
    else if (change_color_num <= 20 * wait_time)
    {
        m_Color.y -= 0.1f / wait_time;
        m_Color.z += 0.1f / wait_time;
    }
    else if (change_color_num <= 30 * wait_time)
    {
        m_Color.z -= 0.1f / wait_time;
        m_Color.x += 0.1f / wait_time;

        if (change_color_num == 30 * wait_time)
        {
            change_color_num = 0;
        }
    }
...
void CBillBoard::SetSRV(ID3D11ShaderResourceView * SRV)
{
    m_srv = SRV;
}
